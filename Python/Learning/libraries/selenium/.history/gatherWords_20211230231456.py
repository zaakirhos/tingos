...
updating_word = list(updating_doc["words"])
print(len(updating_word))
new = updating_word[:]
just_words = list(map(lambda w: w["word"], updating_word))
url = "https://random-word-api.herokuapp.com/word?number=10000"
fetched_words = requests.request("GET", url).json()
cap_fetched_words = list(map(lambda w: w.capitalize(), fetched_words))

diff = getDiff(cap_fetched_words, just_words)
print(len(diff))
for i in range(len(diff)):
    url = f"https://api.dictionaryapi.dev/api/v2/entries/en/{diff[i]}"
    ...
    try:
        ...
        if response.status_code == 429:
            ...
            try:
                ...
                new.append({"word": diff[i], "definition": definition})
            except Exception as e:
                continue
        else:
            i += 1
...
print(len(updating_word), len(new))
wordC.update_one({"_id": ObjectId("61ce1361c1fa56f1ee5367d0")},
                 {"$set": {
                     'words': new, 'count': len(new)
                 }},
                 upsert=True
)
