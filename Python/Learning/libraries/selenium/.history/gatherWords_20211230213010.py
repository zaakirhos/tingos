...
cleaned_Tokens = list(map(clean, tokens))

client = pymongo.MongoClient(
        'mongodb+srv://zakir:mongo123@cluster0.abqkq.mongodb.net/zakir?retryWrites=true&w=majority')
db = client.get_database('pool')
wordC = db.words
existing_words = list(wordC.find({}))

def getWords(token):
    return token["word"]

existing_words = list(map(getWords, existing_words[0]["words"]))
# print(len(existing_words))

def dif(li1, li2):
    return list(set(li1) - set(li2))

diff = dif(existing_words, cleaned_Tokens)
